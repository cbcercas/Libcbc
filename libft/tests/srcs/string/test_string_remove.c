...
Test(string_remove, Test_string_remove_0) {
	...
	cr_expect_eq(rem, 6, "return needed is 6 but your function return %zu", rem);
	cr_expect_eq(str->len, 27, "str->len needed to be 27 after remove, but you have %zu", str->len);
	cr_expect_eq(rem = string_remove(str, 20, 10), 7, "return needed is 7 but your function return %zu", rem);
	string_del(&str);
}
