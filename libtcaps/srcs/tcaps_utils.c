...
/*   Updated: 2017/10/19 06:00:41 by chbravo-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_key_test	g_fn_test[] =
{
	...
	{KEY_CODE_ESC_ENTER, &is_escape_enter},
	{KEY_CODE_NONE, NULL}
};
...
t_key	key_get(const char *buff, BOOL tcaps_activ)
{
	...
	{
		i = -1;
		while (g_fn_test[++i].f != NULL)
		{
			if (g_fn_test[i].f(buff))
				return (key_new(g_fn_test[i].key_code, buff));
		}
	}
	else if ('\n' == buff[0])
